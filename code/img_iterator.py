...
def move_bad_images(directory, new_location=None, type=None):
    ...
    if type == 'year':
        ...
        for month_ in months:
            ...
            for day in month_:
                _day = day.split('/')[-1]
                if not os.path.exists('{0}/{1}/{2}'.format(new_location, _month, _day)):
                    os.makedirs('{0}/{1}/{2}'.format(new_location, _month, _day))
                for img in glob(day + '/*'):
                    _img = img.split('/')[-1]
                    im = Image.open(img)
                    pixels = im.getdata()
                    ...
                    if (nblack / float(n)) > 0.25:
                        os.rename(img, '{0}/{1}/{2}/{3}'.format(new_location, _month, _day, _img))
...
class Iterator(object):
    ...
    def __init__(self, N, batch_size, shuffle, seed):
        ...
        self.index_generator = self._flow_index(N, batch_size, self.shuffle, seed)
...
class DirectoryIterator(Iterator):
    ...
    def next(self):
        ...
        for i, j in enumerate(index_array):
            ...
            img = load_img(img_path, grayscale=grayscale, target_size=self.target_size)
...
class Thesis(object):
    ...
    def __init__(self):
        ...
        self.monthDict={1:'Jan', 2:'Feb', 3:'Mar', 4:'Apr', 5:'May', 6:'Jun', 
    ...
    def classify(self,
        ...
        pool = ThreadPool()
        tmp=Img_Iterator()
        new_dir=self.classified_dir+'{0}/{1}/'.format(self.year,self.month)        
        g=Generator()
    ...
    def map_matrix(self,
        ...
        if dic:
            ...
            np.savetxt('{0}_matrix.csv'.format(self.year), year_matrix, delimiter=',', fmt='%1.1i')
    ...
    def plot_heatmap(self,
        ...
        for month, matrix in dic.items():
            if len(month) > 3:
                xs,ys = self.m(np.arange(-180.,180.,9),np.linspace(90.,-81.,20))
                #xs,ys = self.m(np.arange(-135., -45., 9),np.linspace(54., 18., 5))
                plt.pcolormesh(xs,ys,matrix,alpha=0.8,antialiased=True,vmax=np.max(matrix),vmin=0)
                cbar = plt.colorbar(orientation='horizontal', shrink=0.625,pad=0.000001) # fraction=0.2,
                cbar.set_label('Number of days with transverse bands',size=18, fontname='Times New Roman')
                plt.gcf().set_size_inches(13,8)
                self.m.drawcoastlines()
                self.m.drawcountries()
                parallels=np.linspace(90.,-81.,20)
                meridians=np.arange(-180.,180,18)
                self.m.drawparallels(parallels,labels=[1,0,0,0],fontsize=6)
                self.m.drawmeridians(meridians,labels=[0,0,0,1],fontsize=6)
                #plt.tight_layout()
                plt.title('Transverse band occurence {0}.'.format(self.year),size=22, fontname='Times New Roman')
                plot_dir = os.path.join('/home/jmiller/Dropbox/school/cnn_stuff/heatmaps/climo/new/{0}'.format(self.year)) 
                if not os.path.exists(plot_dir):
                    os.makedirs(plot_dir)
                plt.savefig(os.path.join(plot_dir, '{0}_heatmap.png'.format(month)))
                plt.close('all')
            
            else:
                xs,ys = self.m(np.arange(-180.,180,9),np.linspace(90.,-81,20))
                #xs,ys = self.m(np.arange(-135.,-45.,9),np.linspace(54.,18.,5))
                plt.pcolormesh(xs,ys,matrix,alpha=0.8,antialiased=True,vmax=self.max,vmin=0)
                ...
                self.m.drawmeridians(meridians,labels=[0,0,0,1],fontsize=6)
                #plt.tight_layout()
                plt.title('Transverse band occurrence {0} {1}.'.format(month, self.year), size=22, fontname='Times New Roman')
                plot_dir = os.path.join('/home/jmiller/Dropbox/school/cnn_stuff/heatmaps/climo/new/{0}'.format(self.year)) 
                if not os.path.exists(plot_dir):
                    os.makedirs(plot_dir)
                plt.savefig(os.path.join(plot_dir, '{0}_{1}_heatmap.png'.format(month, self.year)))
                plt.close('all')
    ...
    def genHistogram(self, dic, bounds):
        ...
        plt.title('TCB occurence over the United States in {0}'.format(self.year), size=22, fontname='Times New Roman')
        plt.tight_layout()
        plot_dir = os.path.join('/home/jmiller/Dropbox/school/cnn_stuff/heatmaps/climo/{0}'.format(self.year)) 
        if not os.path.exists(plot_dir):
            os.makedirs(plot_dir)
        plt.savefig(os.path.join(plot_dir, '{0}_histogram.png'.format(self.year)))
    ...
    def genHovmoller(self, dic, bounds):
        ...
        cbar = plt.colorbar(orientation='horizontal', shrink=0.625, pad=0.1, ticks=range(int(over_time.max() + 1)))
        cbar_ticks = [str(i) for i in range(int(over_time.max()) + 1)]
        cbar.ax.set_xticklabels(cbar_ticks, fontname='Times New Roman')
        cbar.set_label('Number of days with transverse bands', size=14, fontname='Times New Roman')
        #plt.gcf().set_size_inches(13,8)
        ...
        plt.title('TCB occurence over the United States in {0}'.format(self.year), size=22, fontname='Times New Roman')
        plt.tight_layout()
        plot_dir = os.path.join('/home/jmiller/Dropbox/school/cnn_stuff/heatmaps/{0}'.format(self.year)) 
        if not os.path.exists(plot_dir):
            os.makedirs(plot_dir)
        plt.savefig(os.path.join(plot_dir, '{0}_hovmoller.png'.format(self.year)))
...
def india(fpath, topLeft=(0, 0), botRight=(0, 0)):
    ...
    for iname in lst:
        ...
        fname=iname.split('/')[-1]
        month=iname.split('/')[-2]
        coords=iname.split('/')[-1].split('-')[-1].split('.')[0].split('_')[2:4]
        print int(coords[0])
