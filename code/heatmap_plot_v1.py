...
def paper_plot(dic):
    ...
    title_size = 16
    im = axes[0].pcolormesh(xs, ys, dic[2013], alpha=0.8, antialiased=True, vmin=0, cmap=cmap)
    m1.drawcoastlines()
    parallels = np.linspace(90., -81., 20)
    meridians = np.arange(-180., 180, 18)
    axes[0].set_title('2013', size=title_size, fontname='Times New Roman')
    axes[0].axis('off')
    cbar0 = fig.colorbar(im, orientation='horizontal', shrink=0.625, ax=axes[0], pad=0.02, )
    ...
    im = axes[1].pcolormesh(xs, ys, dic[2014], alpha=0.8, antialiased=True, vmin=0, cmap=cmap)
    m2.drawcoastlines()
    axes[1].set_title('2014', size=title_size, fontname='Times New Roman')
    axes[1].axis('off')
    cbar1 = fig.colorbar(im, orientation='horizontal', shrink=0.625, ax=axes[1], pad=0.02, )
    ...
    im = axes[2].pcolormesh(xs, ys, dic[2015], alpha=0.8, antialiased=True, vmin=0, cmap=cmap)
    m3.drawcoastlines()
    axes[2].set_title('2015', size=title_size, fontname='Times New Roman')
    axes[2].axis('off')
    cbar2 = fig.colorbar(im, orientation='horizontal', shrink=0.625, ax=axes[2], pad=0.02, )
    ...
    plot_dir = os.path.join('/Users/jmiller/Dropbox/school/cnn_stuff/heatmaps/climo/paper') 
    if not os.path.exists(plot_dir):
        os.makedirs(plot_dir)
    plt.savefig(os.path.join(plot_dir, '3_year_heatmap_.png'), bbox_inches='tight', dpi=300)
