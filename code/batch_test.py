...
import imp
util=imp.load_source('dl_util','/home/jmiller/Dropbox/cnn_stuff/code/dl_util.py')
...
def class_predictions(model, img_lst, batch_size=20):
    ...
    for lst in path_lst:
        ...
        for pred in model.predict_classes(imgs,batch_size=batch_size,verbose=1):
            preds.append(pred)

    return preds

def plot_confusion_matrix(con_matrix, prefix, title='Confusion matrix', cmap=plt.cm.Blues):
    fig,ax=plt.subplots()   
...
if __name__=='__main__':
  ...
  for i in xrange(num_batches):
      current_index=(test_gen.batch_index*test_gen.batch_size)%test_gen.N
