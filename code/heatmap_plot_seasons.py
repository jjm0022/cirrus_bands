...
def map_matrix(dic,
    ...
    season_dict = {'winter':[1,2,3], 'spring':[4,5,6], 'summer':[7,8,9], 'fall':[10,11,12]}
...
def paper_plot(dic):
    ...
    title_size = 16
    im = axes[0,0].pcolormesh(xs, ys, dic['winter'], alpha=0.8, antialiased=True, vmin=0, vmax=max_, cmap='jet')
    m1.drawcoastlines()
    parallels = np.linspace(90., -81., 20)
    meridians = np.arange(-180., 180, 18)
    axes[0,0].set_title('Jan. - Mar.', size=title_size, fontname='Times New Roman')
    axes[0,0].axis('off')
    ...
    im = axes[0,1].pcolormesh(xs, ys, dic['spring'], alpha=0.8, antialiased=True, vmin=0, vmax=max_, cmap='jet')
    m2.drawcoastlines()
    axes[0,1].set_title('Apr. - Jun.', size=title_size, fontname='Times New Roman')
    axes[0,1].axis('off')
    ...
    im = axes[1,0].pcolormesh(xs, ys, dic['summer'], alpha=0.8, antialiased=True, vmin=0, vmax=max_, cmap='jet')
    m3.drawcoastlines()
    axes[1,0].set_title('Jul. - Sep.', size=title_size, fontname='Times New Roman')
    axes[1,0].axis('off')
    ...
    im = axes[1,1].pcolormesh(xs, ys, dic['fall'], alpha=0.8, antialiased=True, vmin=0, vmax=max_, cmap='jet')
    m4.drawcoastlines()
    axes[1,1].set_title('Oct. - Dec.', size=title_size, fontname='Times New Roman')
    axes[1,1].axis('off')
    ...
    #cbar_ax.axis('off')
    plt.tight_layout()
    cbar = fig.colorbar(im, orientation='horizontal', cax=cbar_ax)# pad=0.000001, ) # fraction=0.2,
    cbar.set_label('Number of days with transverse bands', size=16, fontname='Times New Roman')
    ...
    plot_dir = os.path.join('/home/jmiller/Dropbox/school/cnn_stuff/heatmaps/climo/tmp') 
    if not os.path.exists(plot_dir):
        os.makedirs(plot_dir)
    plt.savefig(os.path.join(plot_dir, 'season_heatmap_2015.png'), bbox_inches='tight', dpi=200)
